...
const codeGenerator = function () {
  const code = (Math.floor(1000 + Math.random() * 10000)) % 10000
  return code;
}

const sendMessage = asyncHandler(async (req, res) => {
  const { message, isOnce, mycode } = req.body
  if (!message) {
  ...
  if (!mycode) {
    ...
    const msg = await Message.findOneAndUpdate(
      { code: mycode }, { message: message, isOnce: false }, { new: true, upsert: true, runValidators: true }
    )
    const createdMessage = await Message.findById(msg._id).select("-password");
    if (!createdMessage) {
      throw new apiError(404, "Something went wrong while uploading Message. Try Again")
    ...
    return res.status(201).json(
      new apiResponse(201, createdMessage, ` Your coded Message:${mycode}`)
    )
...
const retrieveMessage = asyncHandler(async (req, res) => {
  ...
  // }
  if (getCode.isOnce === true) {
    await Message.findOneAndDelete({ _id: getCode._id })
  }

  return res.status(200).json(
    new apiResponse(204, null, `${msg}`)
  )
})
